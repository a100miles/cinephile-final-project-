...
async function getProfile(req, res) {
    const user = await User.findById(req.user.id).select('username email avatarUrl favorites');
...
async function updateProfile(req, res) {
    ...
    if (value.avatarUrl !== undefined) user.avatarUrl = value.avatarUrl || '';
...
async function deleteFavorite(req, res) {
    const { source, movieId } = req.params;

    if (!['local', 'tmdb'].includes(source)) {
    ...
    user.favorites = user.favorites.filter((f) => !(f.source === source && String(f.movieId) === String(movieId)));
    ...
    await user.save();
    res.json({ message: 'Favorite removed', favorites: user.favorites.slice(0, 5) });
}

module.exports = { getProfile, updateProfile, toggleFavorite, deleteFavorite };
